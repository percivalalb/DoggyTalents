...
public class ContainerPackPuppy extends Container
{
    ...
    public ContainerPackPuppy(InventoryPlayer par1IInventory, EntityDTDoggy entitydtdoggy)
    {
        ...
        for (int j = 0; j < 3; j++)
        {
            for (int i1 = 0; i1 < wolf.talents.getTalentLevel(EnumSkills.PACKPUPPY); i1++)
            {
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int i)
    {
        ...
        if (slot != null && slot.getHasStack())
        {
            ...
            if (i < 3 * wolf.talents.getTalentLevel(EnumSkills.PACKPUPPY))
            {
                if (!mergeItemStack(itemstack1, 3 * wolf.talents.getTalentLevel(EnumSkills.PACKPUPPY), inventorySlots.size(), true))
                {
                    return null;
                }
            }
            else if (!mergeItemStack(itemstack1, 0, 3 * wolf.talents.getTalentLevel(EnumSkills.PACKPUPPY), false))
            {
                return null;
            }
