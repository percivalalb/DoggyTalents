            ...
            public void run() {
                ...
                while(!checkedVersion && side == Side.CLIENT) {
                	...
                	if(status == OUTDATED) {
                		...
                		if(player != null) {
                			ITextComponent link = ForgeHooks.newChatWithLinks(linkVersion);
                			link.getStyle().setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TextComponentString("Click to download!")));
                			link.getStyle().setColor(TextFormatting.YELLOW);
                			TextComponentTranslation chatComponent = ChatHelper.getChatComponentTranslation("doggytalents.updatemessage", Reference.MOD_NAME, recommendedVersion);
