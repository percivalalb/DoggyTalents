...
public class EntityAIShepherdDog extends EntityAIBase {
	...
	public boolean shouldContinueExecuting() {
		if(!this.dog.MODE.isMode(EnumMode.DOCILE)) {
			...
			return false;
		} else if(!DogUtil.isHolding(this.owner, ModItems.WHISTLE, nbt -> nbt.hasKey("mode") && nbt.getInteger("mode") == 4)) {
			return false;
	...
	public void updateTask() {
		if(!this.dog.isSitting()) {
			...
			if(--this.timeToRecalcPath <= 0) {
				...
				for(EntityAnimal target : this.targets) {
					...
					if(teleport) {
						...
						if(!target.getNavigator().tryMoveToEntityLiving(this.owner, 1.2D))
							if(!target.getLeashed() && target.getRidingEntity() == null && distanceAway >= 20)
								DogUtil.teleportDogToOwner(this.owner, target, this.world, target.getNavigator(), 4);
					}
					else
				...
				if(!this.dogPathfinder.tryMoveToXYZ(j3, this.owner.getEntityBoundingBox().minY, k3, this.followSpeed)) {
					if(this.dog.getDistance(j3, this.owner.getEntityBoundingBox().minY, k3) > 12) {
						if(!this.dog.getLeashed() && this.dog.getRidingEntity() == null)
