...
public class ConfigurationHandler {
	...
	public static void init(Configuration configuration) {
	  ...
	  *
	  *     Also, I would look at versionCheck, since we're now using Forge's version check, I would
	  *     recommend removing this since Forge has this already.
	  *
	...
	public static void loadConfig() {
		...
		//Creates list for doggy settings
		 ...
		 Constants.DIRE_PARTICLES = config.get(CATEGORY_DOGGYSETTINGS, "direParticals", true, "Enables the particle effect on Dire Level 30 dogs.").getBoolean(true);
		...
		orderDSetting.add("direParticals");
		orderDSetting.add("isStartingItemsEnabled");
		orderDSetting.add("bloodWhenIncapacitated");
		
		//Sets the category propetry order to that of which you have set the list above
		config.setCategoryPropertyOrder(CATEGORY_DOGGYSETTINGS, orderDSetting);
		
		
		//Creates list for doggy settings
		List<String> orderDTGeneral = new ArrayList<String>();
		Constants.VERSION_CHECK = config.get(CATEGORY_DT_GENERAL, "versionCheck", true, "Should check whether there are available updates").getBoolean(true);
		
		orderDTGeneral.add("versionCheck");
