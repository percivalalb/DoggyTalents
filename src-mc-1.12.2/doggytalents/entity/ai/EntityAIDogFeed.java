...
public class EntityAIDogFeed extends EntityAIBase {
    ...
    public boolean shouldContinueExecuting() {
        return this.dog.getDogHunger() < Constants.hungerPoints && this.foodEntity != null && this.dog.getDistanceSq(this.foodEntity) < (double)(this.maxDist * this.maxDist) && !this.dog.isSitting();
    }

    @Override
    public void updateTask() {
        if (this.foodEntity != null && this.foodEntity.isEntityAlive()) {
            //this.dog.getNavigator().setPath(this.dog.getNavigator().getPathToXYZ(this.foodEntity.posX, this.foodEntity.posY, this.foodEntity.posZ), 1);
    ...
    public EntityItem getNearestFoodItem() {
    	/*List<EntityItem> foodList = this.world.getEntitiesWithinAABB(EntityItem.class,
                new AxisAlignedBB(this.dog.posX - 30, this.dog.posY - 30, this.dog.posZ - 30,
                        this.dog.posX + 30, this.dog.posY + 30, this.dog.posZ + 30));*/
