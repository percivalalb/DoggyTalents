 ...
 */
public class ContainerPackPuppy2 extends Container {
	
    private EntityDog dog;

    public ContainerPackPuppy2(EntityPlayer player, EntityDog dog) {
        this.dog = dog;
        IInventory inventory = (IInventory)dog.objects.get("farmerinventory");
        inventory.openInventory(player);
        int packpuppyLevel = MathHelper.clamp(this.dog.talents.getLevel("farmerdog"), 0, 1);

        this.addSlotToContainer(new SlotPackPuppy2(inventory, 0, 8, 66, dog));
        /*for (int j = 0; j < 3; j++) {
            for (int i1 = 0; i1 < packpuppyLevel; i1++)
                this.addSlotToContainer(new SlotPackPuppy2(inventory, i1 * 3 + j, 79 + 18 * i1, 1 + 18 * j + 24, dog));
        }*/
