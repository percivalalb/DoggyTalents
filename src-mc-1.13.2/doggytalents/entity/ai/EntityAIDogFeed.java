...
public class EntityAIDogFeed extends EntityAIBase {
	...
	public EntityItem getNearestFoodItem() {
		List<EntityItem> foodList = this.world.getEntitiesWithinAABB(EntityItem.class, dog.getBoundingBox().grow(this.maxDist, 4D, this.maxDist).expand(0.0D, (double) dog.world.getHeight(), 0.0D));
		for (EntityItem foodEntity : foodList) {
			ItemStack stack = foodEntity.getItem();
			int foodValue = this.dog.foodValue(stack);
			boolean isFish = (stack.getItem() == Items.COD || stack.getItem() == Items.COOKED_COD);
	...
	public boolean isEating() {
		return this.foodEntity != null && this.foodEntity.isAlive();
	}
}