			...
			if(passenger instanceof EntityDog) {
				...
				//if(dog.coords.hasBedPos()) {
				    ...
				    if(level == 5 || distance >= 0.0D) {
						...
						AxisAlignedBB boundingBox = new AxisAlignedBB(dog.coords.getBedX(), dog.coords.getBedY(), dog.coords.getBedZ(), dog.coords.getBedX() + 1, dog.coords.getBedY() + 1, dog.coords.getBedZ() + 1).expand(0.5D, 0.5D, 0.5D);
