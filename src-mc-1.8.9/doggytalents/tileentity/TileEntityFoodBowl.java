...
public class TileEntityFoodBowl extends TileEntity implements ITickable {
    ...
    public void update() {
    	...
    	List<EntityDog> dogList = this.worldObj.getEntitiesWithinAABB(EntityDog.class, new AxisAlignedBB(this.pos.getX(), this.pos.getY() + 0.5D, this.pos.getZ(), this.pos.getX() + 1.0D, this.pos.getY() + 0.5D + 0.05000000074505806D, this.pos.getZ() + 1.0D).expand(5D, 5D, 5D));

    	for(EntityDog dog : dogList) {
    		dog.coords.setBowlX(this.pos.getX());
    		dog.coords.setBowlY(this.pos.getY());
    		dog.coords.setBowlZ(this.pos.getZ());
