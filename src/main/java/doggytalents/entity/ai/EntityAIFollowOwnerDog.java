            ...
            if(--this.timeToRecalcPath <= 0) {
                ...
                if(this.isCommanding(this.owner)) { //Holding Sword or tool
                    ...
                    if(diffVec != null) {
                        ...
                        if(!this.dog.getNavigator().tryMoveToXYZ(diffVec.x, this.dog.getPosY(), diffVec.z, this.followSpeed)) {
                            if(!this.dog.getLeashed() && !this.dog.isPassenger())
                                if(distSq >= 350.0D)
                                    DogUtil.teleportDogToOwner(this.owner, this.dog, this.world, this.petPathfinder);
                        }
                        else 
                            this.dog.getLookController().setLookPosition(diffVec.x, this.dog.getPosY() + 1, diffVec.z, 10.0F, (float)this.dog.getVerticalFaceSpeed());
