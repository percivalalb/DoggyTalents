...
public class TalentFeature extends DogFeature implements ITalentFeature {
    ...
    public void writeAdditional(NBTTagCompound compound) {
        ...
        for(Entry<Talent, Integer> entry : map.entrySet()) {
            ...
            subCompound.setString("talent", entry.getKey().getRegistryName().toString());
            subCompound.setInteger("level", entry.getValue());
            list.appendTag(subCompound);
        }

        if(list.tagCount() > 0)
            compound.setTag("talent_level_list", list);
    }
    ...
    public void readAdditional(NBTTagCompound compound) {
        ...
        if(compound.hasKey("talent_level_list", 9)) {

            NBTTagList list = compound.getTagList("talent_level_list", 10);
            for(int i = 0; i < list.tagCount(); i++) {
                NBTTagCompound subCompound = list.getCompoundTagAt(i);
                ResourceLocation talentId = new ResourceLocation(subCompound.getString("talent"));
                int level = subCompound.getInteger("level");
        ...
        } else if(compound.hasKey("talents", 8)) {
