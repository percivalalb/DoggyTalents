...
public class PoisonFangTalent extends Talent {
    ...
    public EnumActionResult onInteract(IDogEntity dog, EntityPlayer player, EnumHand hand) {
        int level = this.getLevel(dog);

        if (dog.isTamed() && level == 5 && dog.getHungerFeature().getDogHunger() > 30) {
            ItemStack stack = player.getHeldItem(hand);
            if(stack.getItem() == Items.SPIDER_EYE && !player.getActivePotionMap().isEmpty()) {
